...
for (var i = 0; i < CONTEXTS.length; i++) {
  ...
  module(context + ' dv.deferred#promise', {
    setup: setup
  });

  test('should return correct promise object (no arg)', function() {
  ...
  test('should always return same promise object (no arg)', function() {
    var p = this.t.promise();
    ok(this.d.promise() === p);
    ok(p.promise() === p);
  });

  test('should return correct populated promise object (with arg)', function() {
    var obj = { a: 1, b: 2 };
    var p = this.t.promise(obj);
    ok(objSize(p) === 8);
    ok(p.done === this.t.done);
    ok(p.fail === this.t.fail);
    ok(p.progress === this.t.progress);
  ...
  module(context + ' dv.deferred#resolve', {
    setup: setup
  });
  ...
  module(context + ' dv.deferred#resolveWith', {
    setup: setup
  });
  ...
  module(context + ' dv.deferred#reject', {
    setup: setup
  });
  ...
  module(context + ' dv.deferred#rejectWith', {
    setup: setup
  });
  ...
  module(context + ' dv.deferred#notify', {
    setup: setup
  });
  ...
  module(context + ' dv.deferred#notifyWith', {
    setup: setup
  });
  ...
  module(context + ' dv.deferred#state', {
    setup: setup
  });
  ...
  module(context + ' dv.deferred#done', {
    setup: setup
  });
  ...
  test('should bind done callbacks provided in any argument, respecting their order, be it an array of functions, or a function', function() {
    ...
    this.t.done(cb[0], [cb[1], cb[2]], [cb[3],
  ...
  module(context + ' dv.deferred#fail', {
    setup: setup
  });
  ...
  test('should bind fail callbacks provided in any argument, respecting their order, be it an array of functions, or a function', function() {
    ...
    this.t.fail(cb[0], [cb[1], cb[2]], [cb[3],
        [cb[4], cb[5], cb[6]], cb[7], cb[8]
      ],
      cb[9]);
    this.d.reject();
  ...
  module(context + ' dv.deferred#always', {
    setup: setup
  });
  ...
  test('should bind always callbacks provided in any argument, respecting their order, be it an array of functions, or a function', function() {
    ...
    this.t.always(cb[0], [cb[1], cb[2]], [cb[3],
        [cb[4], cb[5], cb[6]], cb[7], cb[8]
      ],
      cb[9]);
    this.d.reject();
  ...
  module(context + ' dv.deferred#progress', {
    setup: setup
  });
  ...
  test('should bind progress callbacks provided in any argument, respecting their order, be it an array of functions, or a function', function() {
    ...
    this.t.progress(cb[0], [cb[1], cb[2]], [cb[3],
        [cb[4], cb[5], cb[6]], cb[7], cb[8]
      ],
      cb[9]);
    this.d.notify();
...
test('should return a promise of the deferred received as a single argument)', function() {
  var d = dv.deferred();
  var w = dv.when(d);
  deepEqual(d.promise(), w);
});

test('created deferred should resolve with its subordinates\' values and contexts once all of them get resolved', function() {
  var d1 = dv.deferred();
  var d2 = dv.deferred();
  var d3 = dv.deferred();
  var d4 = dv.deferred();
  ...
  var d = dv.when(d1, d2, d3, d4, d5);
  d.done(function () {
    resolvedValues = args(arguments);
    resolvedContexts = this;
  });

  ok(resolvedValues === undefined);
  ok(resolvedContexts === undefined);
  ...
  d5.resolveWith(Infinity);
  deepEqual(resolvedValues, [1, 2, [3, 3], 4, undefined]);
  deepEqual(resolvedContexts, [null, 'ba', 'cw', null, Infinity]);
});

test('created deferred should fail if any of the subordinates get rejected', function() {
  var d1 = dv.deferred();
  var d2 = dv.deferred();
  var d3 = dv.deferred();
  var d4 = dv.deferred();
  ...
  d.fail(function () {
    'use strict';
    failValues = args(arguments);
    failContexts = this;
  });

  ok(failValues === undefined);
  ok(failContexts === undefined);

  d1.reject(1);
  deepEqual(failValues, [1]);
  deepEqual(failContexts, null);
...
test('created deferred should fail if any of the subordinates get rejected (rejectWith)', function() {
  var d1 = dv.deferred();
  var d2 = dv.deferred();
  var d3 = dv.deferred();
  var d4 = dv.deferred();
...
test('created deferred should progress if any of the subordinates progress', function() {
  var d1 = dv.deferred();
  var d2 = dv.deferred();
  var d3 = dv.deferred();
  var d4 = dv.deferred();
  ...
  d.progress(function () {
    'use strict';
    progressValues = args(arguments);
    progressContexts = this;
  });

  ok(progressValues === undefined);
  ok(progressContexts === undefined);

  d1.notifyWith('az', 1);
  deepEqual([1, undefined, undefined, undefined, undefined], progressValues);
  deepEqual(['az', undefined, undefined, undefined, undefined], progressContexts);

  d2.notifyWith('bq', [2]);
  deepEqual([1, 2, undefined, undefined, undefined], progressValues);
  deepEqual(['az', 'bq', undefined, undefined, undefined], progressContexts);

  d3.notify(3);
  deepEqual([1, 2, 3, undefined, undefined], progressValues);
  deepEqual(['az', 'bq', null, undefined, undefined], progressContexts);

  d4.notifyWith('eg', [4,4]);
  deepEqual([1, 2, 3, [4,4], undefined], progressValues);
  deepEqual(['az', 'bq', null, 'eg', undefined], progressContexts);

  d5.notify(5);
  deepEqual([1, 2, 3, [4,4], 5], progressValues);
  deepEqual(['az', 'bq', null, 'eg', null], progressContexts);

  d1.notifyWith('ax', 11);
  deepEqual([11, 2, 3, [4,4], 5], progressValues);
  deepEqual(['ax', 'bq', null, 'eg', null], progressContexts);
});