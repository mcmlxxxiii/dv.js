 ...
 * dv.js v0.3.0
 *
 * Copyright 2014-2017 by Mykhaylo Gavrylyuk, https://github.com/mcmlxxxiii
...
dv = (function () {
    ...
    function getFunctionSignature(fn) {
        var fnCode = fn.toString(),
            ...
            fnSignature = fnCode.substr(
                    iBracketO + 1, iBracketC - iBracketO - 1),
        ...
        if (arguments.length > 1) {
            ...
            for (i = 0; i < args.length; i++) {
                if (!(args[i] instanceof this.constructor)) {
                    throw new Error('dv: when 2+ args, 2nd (array) should ' +
                        'consist only of dynamic values! ' +
                        '(element '+i+' is not a dv)');
                }
            }
            ...
            this._args = args;
            this._calculationDependsOnOldValues =
                    this._fnSignature.length == this._args.length * 2;
            ...
            if (arguments.length > 2) {
                ...
                this._value =
                        this._fn.apply(this._value, this._argsValues(this));
            }
        }
    ...
    dv.lift = function () {
        ...
        if (arguments.length == 1) {
            ...
            throw new Error('dv.lift: accepts either 1 (liftFn) or ' +
                            '2 (initialValue, liftFn) args');
        }
        ...
        // This hack is needed to make Sinon be able to spy on dv
        // when testing lift.
    ...
    dv.prototype.link = function (dvOther) {
        if (!(dvOther instanceof dv))
            throw new Error('dv#link: accepts other dv as the single ' +
                            'argument only!');
        if (this === dvOther)
    ...
    dv.prototype.map = function () {
        ...
        if (arguments.length == 1) {
            ...
            throw new Error('dv#map: accepts either 1 (mapFn) or ' +
                            '2 (initialValue, mapFn) args');
        }
    ...
    dv.prototype._calculateValue = function (propagatedFrom, forceFlag) {
        ...
        } else if (this._args instanceof Array &&
                   typeof this._fn === 'function') {
            newValue = this._fn.apply(
                    this._value, this._argsValues(propagatedFrom));
            oldValue = this._value;
        }
